module.exports = {
  ...
  checkGuestCounts: function(req, res, next){
    var cart = req.cart;
    if(!cart) return next();
    if(cart.items.some(function(item){ return parseInt(item.guests, 10) > item.vacation.maximumGuests; })){
      if(!cart.errors) cart.errors = [];
      cart.errors.push('One or more of your selected tours cannot accomodate the ' +
